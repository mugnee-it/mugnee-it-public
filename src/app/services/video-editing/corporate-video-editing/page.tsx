...
const PAGE_PATH = "/services/video-editing/corporate-video-editing";
const PAGE_URL = `${SITE_URL}${PAGE_PATH}`;
const KEYWORD = "Corporate Video Editing Service";

export const metadata: Metadata = {
  metadataBase: new URL(SITE_URL),
  title: `${KEYWORD}`,
  description:
    "Corporate video editing service for explainers, brand videos, training, and product demos.",
  alternates: { canonical: PAGE_URL },
  openGraph: {
    title: KEYWORD,
    description: "Professional corporate edits with clean pacing and export-ready delivery.",
    url: PAGE_URL,
    ...
    <main className="bg-white">
      <section className="mx-auto max-w-6xl px-4 pt-16 pb-16">
        <nav className="text-xs font-bold text-slate-500">
          ...
          <span className="mx-2">/</span>
          <span className="text-slate-900">Corporate Editing</span>
        </nav>
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[1.1fr_.9fr] lg:items-center">
          <div>
            ...
            <p className="mt-4 text-base leading-7 text-slate-600">
              Professional edits for explainers, training videos, product demos, and brand stories
              with clean pacing and polished audio.
            </p>

            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              <Bullet text={`${KEYWORD} clean pacing`} />
              <Bullet text={`${KEYWORD} brandâ€‘safe style`} />
          ...
          <div className="rounded-3xl border bg-slate-50 p-6">
            ...
            <div className="mt-4 space-y-3 text-sm text-slate-700">
              <Mini text={`${KEYWORD} professional look`} />
              <Mini text={`${KEYWORD} brand consistency`} />
              <Mini text={`${KEYWORD} clear messaging`} />
              <Mini text={`${KEYWORD} organized delivery`} />
            </div>
          </div>
        </div>
      </section>
      ...
      <Section
        ...
        items={[
          "Share script/footage",
          "Structure + edit pacing",
          "Polish audio + graphics",
          "Export + delivery",
        ]}
      />
      ...
      <Faq
        title={`${KEYWORD} FAQ`}
        items={[
          ["Do you edit training videos?", "Yes, training and explainer videos are supported."],
          ["Do you add graphics?", "Basic motion graphics can be included."],
          ["Do you include captions?", "Yes, captions are available."],
          ["Can you deliver in batches?", "Yes, batch delivery is supported."],
        ]}
      />
    </main>
  );
