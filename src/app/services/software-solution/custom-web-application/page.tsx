...
const PAGE_PATH = "/services/software-solution/custom-web-application";
const PAGE_URL = `${SITE_URL}${PAGE_PATH}`;
const KEYWORD = "Custom Web Application";

export const metadata: Metadata = {
  metadataBase: new URL(SITE_URL),
  title: `${KEYWORD}`,
  description:
    "Custom web application development for secure, scalable, and high-performance systems.",
  alternates: { canonical: PAGE_URL },
  openGraph: {
    title: KEYWORD,
    description: "Build web applications tailored to your workflow and users.",
    url: PAGE_URL,
    ...
    <main className="bg-white">
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[1.1fr_.9fr] lg:items-center">
          <div>
            ...
            <p className="mt-4 text-base leading-7 text-slate-600">
              We build custom web applications with secure access, scalable architecture, and clean
              UI so your users can work efficiently.
            </p>

            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              <Bullet text={`${KEYWORD} scalable architecture`} />
              <Bullet text={`${KEYWORD} secure access`} />
              <Bullet text={`${KEYWORD} clean UI`} />
              <Bullet text={`${KEYWORD} performance-ready`} />
            </div>
          ...
          <div className="rounded-3xl border bg-slate-50 p-6">
            ...
            <div className="mt-4 space-y-3 text-sm text-slate-700">
              <Mini text={`${KEYWORD} fast performance`} />
              <Mini text={`${KEYWORD} secure workflows`} />
              <Mini text={`${KEYWORD} scalable modules`} />
              <Mini text={`${KEYWORD} easy maintenance`} />
            </div>
          </div>
        </div>
      </section>
      ...
      <Section
        ...
        items={[
          ...
          "Design UI and system structure",
          "Build modules and APIs",
      ...
      <Faq
        title={`${KEYWORD} FAQ`}
        items={[
          ["Can you build admin panels?", "Yes, admin panels are included if needed."],
          ["Is it mobile-friendly?", "Yes, the UI is responsive by default."],
          ["Do you handle hosting?", "Yes, we can deploy to VPS or cloud."],
          ["Can you add integrations?", "Yes, we support APIs and integrations."],
        ]}
      />
    </main>
  );
