...
const PAGE_PATH = "/services/software-solution/internal-management-system";
const PAGE_URL = `${SITE_URL}${PAGE_PATH}`;
const KEYWORD = "Internal Management System";

export const metadata: Metadata = {
  metadataBase: new URL(SITE_URL),
  title: `${KEYWORD}`,
  description:
    "Internal management system with role-based access, dashboards, and workflow control.",
  alternates: { canonical: PAGE_URL },
  openGraph: {
    title: KEYWORD,
    description: "Centralize teams and operations with a role-based internal system.",
    url: PAGE_URL,
...
export default function InternalManagementSystemPage() {
  return (
    <main className="bg-white">
      <section className="mx-auto max-w-6xl px-4 pt-16 pb-16">
        <nav className="text-xs font-bold text-slate-500">
          ...
          <span className="mx-2">/</span>
          <span className="text-slate-900">Internal Management</span>
        </nav>
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[1.1fr_.9fr] lg:items-center">
          <div>
            ...
            <p className="mt-4 text-base leading-7 text-slate-600">
              We build internal management systems with dashboards, role-based access, and workflows
              so your teams can work faster with less confusion.
            </p>

            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              <Bullet text={`${KEYWORD} role-based access`} />
              <Bullet text={`${KEYWORD} team dashboards`} />
              <Bullet text={`${KEYWORD} approvals flow`} />
              <Bullet text={`${KEYWORD} reporting system`} />
            </div>
          ...
          <div className="rounded-3xl border bg-slate-50 p-6">
            ...
            <div className="mt-4 space-y-3 text-sm text-slate-700">
              <Mini text={`${KEYWORD} centralized operations`} />
              <Mini text={`${KEYWORD} faster task flow`} />
              <Mini text={`${KEYWORD} clear accountability`} />
              <Mini text={`${KEYWORD} secure permissions`} />
            </div>
          </div>
        </div>
      </section>
      ...
      <Section
        ...
        items={[
          "Map team roles and workflows",
          "Design dashboards and access levels",
          "Build modules and reporting",
          "Test and deploy for teams",
        ]}
      />
      ...
      <Faq
        title={`${KEYWORD} FAQ`}
        items={[
          ["Can you add multiple roles?", "Yes, we build multi-role systems."],
          ["Is the system secure?", "Yes, we use secure access controls."],
          ["Can you add reporting?", "Yes, dashboards and exports are included."],
          ["Do you support after launch?", "Yes, support plans are available."],
        ]}
      />
    </main>
  );
