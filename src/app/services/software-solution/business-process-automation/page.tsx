...
const PAGE_PATH = "/services/software-solution/business-process-automation";
const PAGE_URL = `${SITE_URL}${PAGE_PATH}`;
const KEYWORD = "Business Process Automation";

export const metadata: Metadata = {
  metadataBase: new URL(SITE_URL),
  title: `${KEYWORD}`,
  description:
    "Business process automation to reduce manual work, speed approvals, and improve accuracy.",
  alternates: { canonical: PAGE_URL },
  openGraph: {
    title: KEYWORD,
    description: "Automate tasks and approvals with reliable workflow systems.",
    url: PAGE_URL,
    ...
    <main className="bg-white">
      <section className="mx-auto max-w-6xl px-4 pt-16 pb-16">
        <nav className="text-xs font-bold text-slate-500">
          ...
          <span className="mx-2">/</span>
          <span className="text-slate-900">Process Automation</span>
        </nav>
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[1.1fr_.9fr] lg:items-center">
          <div>
            ...
            <p className="mt-4 text-base leading-7 text-slate-600">
              We automate repetitive workflows with approvals, notifications, and rules so your
              business runs faster and with fewer errors.
            </p>

            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              <Bullet text={`${KEYWORD} workflow mapping`} />
              <Bullet text={`${KEYWORD} approvals setup`} />
              <Bullet text={`${KEYWORD} notifications`} />
              <Bullet text={`${KEYWORD} error reduction`} />
            </div>
          ...
          <div className="rounded-3xl border bg-slate-50 p-6">
            ...
            <div className="mt-4 space-y-3 text-sm text-slate-700">
              <Mini text={`${KEYWORD} faster approvals`} />
              <Mini text={`${KEYWORD} fewer manual steps`} />
              <Mini text={`${KEYWORD} clear tracking`} />
              <Mini text={`${KEYWORD} consistent results`} />
            </div>
          </div>
        </div>
      </section>
      ...
      <Section
        ...
        items={[
          ...
          "Design automated steps and roles",
          "Build rules, approvals, and triggers",
      ...
      <Section
        ...
        items={[
          ...
          "Approval and role rules",
          "Notification and tracking setup",
      ...
      <Faq
        title={`${KEYWORD} FAQ`}
        items={[
          ["Can you automate approvals?", "Yes, we build approval and notification rules."],
          ["Will it reduce manual work?", "Yes, automation removes repetitive steps."],
          ["Is it secure?", "Yes, roles and permissions are included."],
          ["Can you integrate email/SMS?", "Yes, we can integrate notifications."],
        ]}
      />
    </main>
  );
