...
const PAGE_PATH = "/services/software-solution/erp-inventory-solution";
const PAGE_URL = `${SITE_URL}${PAGE_PATH}`;
const KEYWORD = "ERP & Inventory Solution";

export const metadata: Metadata = {
  metadataBase: new URL(SITE_URL),
  title: `${KEYWORD}`,
  description:
    "ERP and inventory solution for stock tracking, approvals, reporting, and multi-role access.",
  alternates: { canonical: PAGE_URL },
  openGraph: {
    title: KEYWORD,
    description: "Manage inventory, modules, and reports with a custom ERP solution.",
    url: PAGE_URL,
...
export default function ErpInventorySolutionPage() {
  return (
    <main className="bg-white">
      <section className="mx-auto max-w-6xl px-4 pt-16 pb-16">
        <nav className="text-xs font-bold text-slate-500">
          ...
          <span className="mx-2">/</span>
          <span className="text-slate-900">ERP & Inventory</span>
        </nav>
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[1.1fr_.9fr] lg:items-center">
          <div>
            ...
            <p className="mt-4 text-base leading-7 text-slate-600">
              We build ERP and inventory solutions with stock tracking, approvals, multi-role access,
              and reporting so operations stay accurate and fast.
            </p>

            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              <Bullet text={`${KEYWORD} stock tracking`} />
              <Bullet text={`${KEYWORD} approvals flow`} />
              <Bullet text={`${KEYWORD} role-based access`} />
              <Bullet text={`${KEYWORD} reporting modules`} />
            </div>
          ...
          <div className="rounded-3xl border bg-slate-50 p-6">
            ...
            <div className="mt-4 space-y-3 text-sm text-slate-700">
              <Mini text={`${KEYWORD} real-time stock`} />
              <Mini text={`${KEYWORD} fewer errors`} />
              <Mini text={`${KEYWORD} clear approvals`} />
              <Mini text={`${KEYWORD} better reporting`} />
            </div>
          </div>
        </div>
      </section>
      ...
      <Section
        ...
        items={[
          "Map inventory workflows and roles",
          "Design modules and approval rules",
      ...
      <Faq
        title={`${KEYWORD} FAQ`}
        items={[
          ["Can you customize modules?", "Yes, modules are built per your workflow."],
          ["Do you handle approvals?", "Yes, we add approval and audit flows."],
          ["Is it scalable?", "Yes, the system scales with your operations."],
          ["Do you support after launch?", "Yes, maintenance is available."],
        ]}
      />
    </main>
  );
