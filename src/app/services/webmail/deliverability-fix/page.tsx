...
const PAGE_PATH = "/services/webmail/deliverability-fix";
const PAGE_URL = `${SITE_URL}${PAGE_PATH}`;
const KEYWORD = "Email Deliverability Fix";

export const metadata: Metadata = {
  metadataBase: new URL(SITE_URL),
  title: `${KEYWORD}`,
  description:
    "Email deliverability fix service: diagnose spam issues, fix authentication, and improve inbox placement.",
  alternates: { canonical: PAGE_URL },
  openGraph: {
    title: KEYWORD,
    description: "Fix spam issues and improve inbox placement safely.",
    url: PAGE_URL,
...
export default function DeliverabilityFixPage() {
  return (
    <main className="bg-white">
      <section className="mx-auto max-w-6xl px-4 pt-16 pb-16">
        <nav className="text-xs font-bold text-slate-500">
          ...
          <span className="mx-2">/</span>
          <span className="text-slate-900">Deliverability Fix</span>
        </nav>
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[1.1fr_.9fr] lg:items-center">
          <div>
            ...
            <p className="mt-4 text-base leading-7 text-slate-600">
              We diagnose authentication issues and sending patterns to improve inbox
              placement and reduce spam complaints.
            </p>

            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              <Bullet text={`${KEYWORD} diagnostics`} />
              <Bullet text={`${KEYWORD} DNS fixes`} />
          ...
          <div className="rounded-3xl border bg-slate-50 p-6">
            ...
            <div className="mt-4 space-y-3 text-sm text-slate-700">
              <Mini text={`${KEYWORD} inbox trust`} />
              <Mini text={`${KEYWORD} fewer bounces`} />
              <Mini text={`${KEYWORD} better reputation`} />
              <Mini text={`${KEYWORD} clear guidance`} />
            </div>
          </div>
        </div>
      </section>
      ...
      <Section
        ...
        items={[
          ...
          "Audit DNS conflicts and routing",
          "Review sending patterns",
          "Provide fixes + verification",
        ]}
      />
      ...
      <Faq
        title={`${KEYWORD} FAQ`}
        items={[
          ["Why emails go to spam?", "Often due to SPF/DKIM/DMARC or sending behavior."],
          ["Can you fix Outlook/Gmail?", "We improve authentication and reputation signals."],
          ["Do you provide verification?", "Yes, we verify headers and DNS."],
          ["Do you offer ongoing support?", "Yes, monitoring and followâ€‘ups are available."],
        ]}
      />
    </main>
  );
