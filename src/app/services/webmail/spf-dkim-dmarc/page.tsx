...
const PAGE_PATH = "/services/webmail/spf-dkim-dmarc";
const PAGE_URL = `${SITE_URL}${PAGE_PATH}`;
const KEYWORD = "SPF DKIM DMARC Setup";

export const metadata: Metadata = {
  metadataBase: new URL(SITE_URL),
  title: `${KEYWORD}`,
  description:
    "SPF DKIM DMARC setup service for email authentication, spam reduction, and inbox trust.",
  alternates: { canonical: PAGE_URL },
  openGraph: {
    title: KEYWORD,
    description: "Email authentication setup with verified DNS records.",
    url: PAGE_URL,
    ...
    <main className="bg-white">
      <section className="mx-auto max-w-6xl px-4 pt-16 pb-16">
        <nav className="text-xs font-bold text-slate-500">
          ...
          <span className="mx-2">/</span>
          <span className="text-slate-900">SPF DKIM DMARC</span>
        </nav>
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[1.1fr_.9fr] lg:items-center">
          <div>
            ...
            <p className="mt-4 text-base leading-7 text-slate-600">
              We configure SPF, DKIM, and DMARC for your domain to reduce spam issues
              and improve inbox placement.
            </p>

            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              <Bullet text={`${KEYWORD} DNS records`} />
              <Bullet text={`${KEYWORD} verification`} />
              <Bullet text={`${KEYWORD} spam reduction`} />
              <Bullet text={`${KEYWORD} reporting setup`} />
            </div>
          ...
          <div className="rounded-3xl border bg-slate-50 p-6">
            ...
            <div className="mt-4 space-y-3 text-sm text-slate-700">
              ...
              <Mini text={`${KEYWORD} reduced spam`} />
              <Mini text={`${KEYWORD} safer sending`} />
      ...
      <Section
        ...
        items={[
          "Responsible DNS updates",
          "Spam-safe recommendations",
          "Reâ€‘run support",
          "Clear documentation",
        ]}
      />
      ...
      <Faq
        title={`${KEYWORD} FAQ`}
        items={[
          ["Do you check existing SPF?", "Yes, to avoid conflicts."],
          ["Can you add DMARC reports?", "Yes, we can set reporting emails."],
          ["Does this stop spam fully?", "It improves trust and deliverability."],
          ["Do you verify setup?", "Yes, we test headers for pass status."],
        ]}
      />
    </main>
  );
